...
public class Main {
	...
	static Function rosenbrock = new Function() {
	...
	static Function eggcrate = new Function() {
...
//		fireFly();
	...
	private static void downHillSimplex() throws Exception
	{
		Function f = himmelblau;
		DownhillSimplex fireFlyAlgorithm = new DownhillSimplex(f, 0.001, 20, 0.9, 0.5);
		fireFlyAlgorithm.findMin(new double[]{0,0});
		XYContourPlot plot = new XYContourPlot("DownhillSimplex", "x1", "x2", 800, 600, f, -5, 5, 0.1, 10);
		plot.addDataSet("steps", fireFlyAlgorithm.getHist());
		plot.showChart();
	}
	
	private static void fireFly() throws Exception
	{
		FireFlyAlgorithm fireFlyAlgorithm = new FireFlyAlgorithm(eggcrate, new double[][]{{-1,4},{-1,4}});
		fireFlyAlgorithm.setPathToSave("/home/linux/Schreibtisch/MA/");
		fireFlyAlgorithm.setNumberOfFlies(100);
		fireFlyAlgorithm.setShouldPlot(true);
		fireFlyAlgorithm.findMin();
		fireFlyAlgorithm.printAllSolution();
	}
	
	private static void integralTest() throws Exception
	{
		...
		f.integrate(new double[]{0,0}, new double[]{2,2});
	}
